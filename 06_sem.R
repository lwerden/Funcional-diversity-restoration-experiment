...
sem_data <- ht %>%
  ...
  mutate(sp_richness = case_when(
    Treatment == "M1" ~ 1, Treatment == "M2" ~ 1,
    Treatment == "2SP" ~ 2, Treatment == "6SP" ~ 6, Treatment == "12SP" ~ 12
  ))

sem_df <- sem_data %>%
  select(Site, sp_richness, mean_height, C_Mg_ha, canopy_cover,
         height_gini, canopy_gini) %>%
  mutate(Site = as.factor(Site)) %>%
  mutate(across(c(sp_richness:canopy_gini), ~ as.numeric(scale(.)))) %>%
  as.data.frame()

# ============================================================
# Fit piecewise SEM
# ============================================================
m1 <- lmer(height_gini ~ sp_richness + (1 | Site), data = sem_df)
m2 <- lmer(C_Mg_ha ~ sp_richness + (1 | Site), data = sem_df)
m3 <- lmer(canopy_cover ~ sp_richness + height_gini + C_Mg_ha + (1 | Site), data = sem_df)
m4 <- lmer(canopy_gini ~ sp_richness + canopy_cover + (1 | Site), data = sem_df)
...
b1 <- get_beta("height_gini", "sp_richness")
b2 <- get_beta("C_Mg_ha", "sp_richness")
b3 <- get_beta("canopy_cover", "sp_richness")
...
edges <- edges %>%
  mutate(
    ...
    color = ifelse(pval >= 0.05, "grey70", ifelse(beta > 0, "#009E73", "#D55E00")),
    lwd = ifelse(pval >= 0.05, 0.4, abs(beta) * 2.5 + 0.5),
    ltype = ifelse(pval >= 0.05, "dashed", "solid"),
    label = paste0(round(beta, 2), sig),
    mid_x = (from_x + to_x) / 2, mid_y = (from_y + to_y) / 2,
    label_y = mid_y + c(0.22, -0.22, 0.2, 0.22, -0.22, 0.2))
...
fig5 <- ggplot() +
  ...
  geom_label(data = edges,
    aes(x = mid_x, y = label_y, label = label),
    size = 3.5, fill = "white", label.size = 0,
    label.padding = unit(0.15, "lines"),
    color = edges$color, fontface = "bold") +
  ...
  geom_text(data = nodes, aes(x = x, y = y, label = name),
    size = 3.2, lineheight = 0.85, fontface = "bold") +
...
ggsave("fig5_sem.png", plot = fig5, width = 9, height = 5, dpi = 300)
