library(dplyr)
library(ggplot2)
library(ggrepel)
library(ineq)
library(BIOMASS)
library(patchwork)
library(fmsb)
library(png)
library(grid)

source("00_preprocess.R")

trt_colors <- c("M1" = "#D55E00", "M2" = "#CC79A7",
                "2SP" = "#56B4E9", "6SP" = "#009E73", "12SP" = "#0072B2")

# ============================================================
# Compute plot-level metrics
# ============================================================
ht_plot <- TreeData %>%
  filter(!is.na(Height_year_3)) %>%
  group_by(Site, Treatment, Plot_number) %>%
  summarise(mean_height = mean(Height_year_3), .groups = "drop")

bio <- TreeData %>%
  filter(!is.na(DBH_year_3), !is.na(Height_year_3), Height_year_3 > 0, !is.na(WD))
bio$AGB_Mg <- computeAGB(D = bio$DBH_year_3, WD = bio$WD, H = bio$Height_year_3 / 100)
c_plot <- bio %>%
  group_by(Site, Treatment, Plot_number) %>%
  summarise(C_Mg_ha = sum(AGB_Mg) / 0.0225 * 0.47, .groups = "drop")

gini_ht <- TreeData %>%
  filter(!is.na(Height_year_3)) %>%
  group_by(Site, Treatment, Plot_number) %>%
  summarize(height_gini = ineq(Height_year_3, type = "Gini"), .groups = "drop")

cc_plot <- CanopyCover %>%
  filter(Year == "3") %>%
  group_by(Site, Treatment, Plot_number) %>%
  summarise(canopy_cover = mean(Percent_canopy_cover, na.rm = TRUE), .groups = "drop")

gini_cc <- CanopyCover %>%
  filter(Year == "3") %>%
  group_by(Site, Treatment, Plot_number) %>%
  summarize(canopy_gini = ineq(Percent_canopy_cover, type = "Gini"), .groups = "drop")

surv <- TreeData %>%
  group_by(Site, Treatment, Plot_number) %>%
  summarise(survival = sum(!is.na(Height_year_3)) / n() * 100, .groups = "drop")

plot_metrics <- ht_plot %>%
  inner_join(c_plot, by = c("Site", "Treatment", "Plot_number")) %>%
  inner_join(gini_ht, by = c("Site", "Treatment", "Plot_number")) %>%
  inner_join(surv, by = c("Site", "Treatment", "Plot_number")) %>%
  left_join(cc_plot, by = c("Site", "Treatment", "Plot_number")) %>%
  left_join(gini_cc, by = c("Site", "Treatment", "Plot_number")) %>%
  filter(!is.na(canopy_gini))

# ============================================================
# RADAR PLOT
# ============================================================
radar_data <- plot_metrics %>%
  group_by(Treatment) %>%
  summarise(
    `Mean\nheight` = mean(mean_height), `Carbon\n(Mg C/ha)` = mean(C_Mg_ha),
    `Canopy\ncover` = mean(canopy_cover), `Height\nheterogeneity` = mean(height_gini),
    `Canopy\nheterogeneity` = mean(canopy_gini), `Survival` = mean(survival),
    .groups = "drop")

treatments <- as.character(radar_data$Treatment)
radar_norm <- radar_data %>%
  select(-Treatment) %>%
  mutate(across(everything(), ~ (. - min(.)) / (max(.) - min(.))))
radar_fmsb <- rbind(rep(1, 6), rep(0, 6), radar_norm)

png("fig4a_radar.png", width = 6, height = 6, units = "in", res = 300)
par(mar = c(1, 1.5, 1.5, 1.5), family = "sans")
radarchart(radar_fmsb, axistype = 1,
  pcol = unname(trt_colors), pfcol = adjustcolor(unname(trt_colors), alpha.f = 0.08),
  plwd = 2.5, plty = 1, cglcol = "grey75", cglty = 1, cglwd = 0.8,
  axislabcol = "grey40", vlcex = 0.95, calcex = 0.7,
  caxislabels = c("0", "0.25", "0.5", "0.75", "1"))
legend("topright", legend = treatments, col = unname(trt_colors),
       lwd = 2.5, bty = "n", cex = 0.9)
dev.off()

radar_gg <- ggplot() +
  annotation_custom(rasterGrob(readPNG("fig4a_radar.png"), interpolate = TRUE)) +
  theme_void() + labs(tag = "a")

# ============================================================
# PCA
# ============================================================
pca_mat <- plot_metrics %>%
  select(mean_height, C_Mg_ha, canopy_cover, height_gini, canopy_gini, survival) %>%
  scale()
colnames(pca_mat) <- c("Mean height", "Carbon", "Canopy cover",
                        "Height heterog.", "Canopy heterog.", "Survival")
pca_result <- prcomp(pca_mat)
ve <- summary(pca_result)$importance[2, ]

scores <- as.data.frame(pca_result$x[, 1:2])
scores$Treatment <- plot_metrics$Treatment
scores$Site <- plot_metrics$Site

loadings <- as.data.frame(pca_result$rotation[, 1:2])
loadings$Variable <- rownames(loadings)

centroids <- scores %>%
  group_by(Treatment) %>%
  summarise(PC1 = mean(PC1), PC2 = mean(PC2), .groups = "drop")

pca_plot <- ggplot(scores, aes(x = PC1, y = PC2)) +
  stat_ellipse(aes(color = Treatment, fill = Treatment),
               geom = "polygon", alpha = 0.05, level = 0.95,
               linewidth = 0.5, linetype = "dashed") +
  geom_point(aes(color = Treatment, shape = Site), size = 2, alpha = 0.6) +
  geom_point(data = centroids, aes(color = Treatment),
             size = 6, shape = 18, alpha = 0.5) +
  geom_text_repel(data = centroids, aes(label = Treatment, color = Treatment),
                  size = 4, fontface = "bold", box.padding = 0.5,
                  point.padding = 0.3, show.legend = FALSE) +
  geom_segment(data = loadings,
               aes(x = 0, y = 0, xend = PC1 * 3, yend = PC2 * 3),
               arrow = arrow(length = unit(0.2, "cm")),
               color = "grey30", linewidth = 0.5) +
  geom_text_repel(data = loadings,
                  aes(x = PC1 * 3.3, y = PC2 * 3.3, label = Variable),
                  size = 3, color = "grey20", fontface = "italic",
                  max.overlaps = 15) +
  scale_color_manual(values = trt_colors) +
  scale_fill_manual(values = trt_colors) +
  scale_shape_manual(values = c("FAB" = 16, "FCEA" = 17, "GAM" = 15, "LLFS" = 3)) +
  labs(x = paste0("PC1 (", round(ve[1] * 100, 1), "%)"),
       y = paste0("PC2 (", round(ve[2] * 100, 1), "%)"),
       shape = "Site", tag = "b") +
  guides(color = "none", fill = "none") +
  theme_minimal() +
  theme(legend.position = "right")

# ============================================================
# COMBINED FIG 4
# ============================================================
fig4 <- radar_gg + pca_plot + plot_layout(widths = c(1, 1.2))
ggsave("fig4_synthesis.png", plot = fig4, width = 13, height = 6, dpi = 300)
file.remove("fig4a_radar.png")
cat("Saved: fig4_synthesis.png\n")
